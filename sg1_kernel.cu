    ...
    int max_mismatch)
{
    ...
    if (q_idx < num_queries && c_idx < num_components) {
        ...
        // SG1 mismatch transform in packed 2-bit space.
        // diff encodes per-bit differences; folding with right-shift and mask
        // reduces each 2-bit nucleotide pair to one mismatch bit.
        uint64_t diff = q ^ c;
        uint64_t norm = (diff | (diff >> 1)) & 0x5555555555555555ULL;
        int dist = __popcll(norm);

        // Store mismatch count when within threshold, otherwise mark as safe.
        output_matrix[q_idx * num_components + c_idx] =
            (dist <= max_mismatch) ? dist : -1;
    }
}
